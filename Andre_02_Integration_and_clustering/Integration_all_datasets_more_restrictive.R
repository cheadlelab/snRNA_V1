...
setwd("/grid/cheadle/home/machado/Projects/Microglia_sensory_experience_genomics/snRNAseq_all_nuclei_V1/New_pipeline-snRNAseq/R-analysis_Lin_align/02_Integration_and_clustering/more_restrictive/")
...
merged_filtered <- subset(x = merged_filtered, 
              subset= (nCount_decontXcounts >= 250) & nFeature_decontXcounts >= 150)
dim(merged_filtered)
...
setwd("/grid/cheadle/home/machado/Projects/Microglia_sensory_experience_genomics/snRNAseq_all_nuclei_V1/New_pipeline-snRNAseq/R-analysis_Lin_align/02_Integration_and_clustering/more_restrictive/no_batch_correction")

#SCT normalization
Allexp.sct_nobatch_corrected <- SCTransform(merged_filtered_cleaned, verbose = FALSE, vars.to.regress = "percent.mt", variable.features.n = 3000, assay = "decontXcountsclean", new.assay.name = "sctdecontXcountsclean")
...
setwd("/grid/cheadle/home/machado/Projects/Microglia_sensory_experience_genomics/snRNAseq_all_nuclei_V1/New_pipeline-snRNAseq/R-analysis_Lin_align/02_Integration_and_clustering/more_restrictive/batch_correction")

#In case of batch effects, proceed with this code
#Splitting dataset, normalizing and scaling for RedDim
merged_filtered_cleaned_list <- SplitObject(merged_filtered_cleaned, split.by = "batch")
merged_filtered_cleaned_list_sct <- lapply(X = merged_filtered_cleaned_list, FUN = function(x) {
...
features <- SelectIntegrationFeatures(object.list = merged_filtered_cleaned_list_sct, nfeatures = 3000)
merged_filtered_cleaned_list_sct <- PrepSCTIntegration(object.list = merged_filtered_cleaned_list_sct, anchor.features = features)
all.nuclei.anchors <- FindIntegrationAnchors(object.list = merged_filtered_cleaned_list_sct, reference = 1, normalization.method = "SCT",
...
ggsave(UMAPAllexp.sct_batch_corrected.40nres4, filename = "UMAPAllexp.sct_batch_corrected.40nres4.png", height = 7, width = 7)
