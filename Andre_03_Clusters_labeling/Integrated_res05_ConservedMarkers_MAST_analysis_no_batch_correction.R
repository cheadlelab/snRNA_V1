...
MAST_res05_top20 <- conserved_markers %>% 
...
MAST_res05_top3 <- conserved_markers %>% 
  mutate(avg_fc = (NR_avg_log2FC + LDR_avg_log2FC + LDR30m_avg_log2FC + LDR2h_avg_log2FC + LDR4h_avg_log2FC + LDR6h_avg_log2FC) /6) %>% 
  group_by(cluster_id) %>% 
  top_n(n = 3, wt = avg_fc) %>%
  .$gene

write.csv(MAST_res05_top20, file = "MAST_res05_top20_conserved_markers_final.csv")

MAST_res05_top20_filtered <- MAST_res05_top20 %>% dplyr::filter(NR_p_val_adj < 0.01 & NR_pct.1 > 0.7 & NR_pct.2 < 0.3)

write.csv(MAST_res05_top20_filtered, file = "MAST_res05_top20_filtered_conserved_markers_final.csv")

write.csv(MAST_res05_top3, file = "MAST_res05_top3_genes.csv")
